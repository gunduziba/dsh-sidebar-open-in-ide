 ...
 * - Main window: every collapsed tool-card row (read/write/edit "product
 *   file") gets a hover-revealed "idea" pill next to `<hash>_fileLink`. The
 *   link's textContent is the path summary — absolute when the path is
 *   outside the workspace, cwd-relative otherwise — and the reveal rule keys
 *   off the stable `data-tool` attribute, so this part has no hash dependency.
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Main-window tool cards (dsh-client-ui-tool): collapsed rows render
    // `button.<hash>_fileLink` — the "product file" link. Its textContent is
    // the path summary (absolute when outside the workspace, cwd-relative
    // otherwise), and the card root carries the stable `data-tool` attribute,
    // so the hover-reveal CSS below has zero hash dependency.
    const FILE_LINK_LOCAL = 'fileLink';
    const MAIN_BTN_CLASS = 'mainInIdeBtn';
    const LABEL = 'idea';
    ...
    const CSS = [
      ...
      // main-window tool-card product-file pill; revealed on card hover via the
      // stable `data-tool` attribute (hash-independent), like the built-in
      // inspect button
      '.' + MAIN_BTN_CLASS + '{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:20px;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;align-items:center;align-self:center;padding:0 8px;display:none}',
      '.' + MAIN_BTN_CLASS + ':hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}',
      '[data-tool]:hover .' + MAIN_BTN_CLASS + ',[data-tool]:focus-within .' + MAIN_BTN_CLASS + '{display:inline-flex}',
      '.' + MAIN_BTN_CLASS + '.openInIdeOk{color:var(--dsw-alias-state-success-primary)}',
      '.' + MAIN_BTN_CLASS + '.openInIdeErr{color:var(--dsw-alias-state-error-primary);border-color:var(--dsw-alias-state-error-primary)}',
      '.' + MAIN_BTN_CLASS + ':disabled{opacity:.5;cursor:default}',
    ...
    // ── main-window tool-card product-file button injection ──
    // Collapsed tool rows render `button.<hash>_fileLink`; its textContent is
    // the path summary. Summary starts with '/' → already absolute (outside
    // the workspace). Otherwise it is workspace-relative → resolve against the
    // session cwd (same rule as dsh-client-ui-tool lib/client.js:126).
    function resolveMainPath(summary) {
      const s = (summary || '').trim();
      if (!s) return null;
      if (s.startsWith('/')) return s;
      const cwd = currentCwd();
      if (!cwd) return null;
      return (cwd.replace(/\/+$/, '') + '/' + s).replace(/\/{2,}/g, '/');
    }

    function injectMainButtons() {
      const links = findLocal(FILE_LINK_LOCAL);
      for (const link of links) {
        if (link.dataset.oiiMain) continue; // already handled
        link.dataset.oiiMain = '1';
        const summary = link.textContent || '';
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = MAIN_BTN_CLASS;
        btn.textContent = LABEL;
        btn.title = '在 IntelliJ IDEA 中打开';
        btn.setAttribute('aria-label', '在 IntelliJ IDEA 中打开');
        btn.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          const path = resolveMainPath(summary);
          if (!path) {
            showToast('无法确定文件路径（会话工作目录不可用）', 'err');
            flash(btn, 'err', '✕', '无法确定文件路径', 2600);
            return;
          }
          openInIde(path, btn);
        });
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        return () => {
          ...
          document.querySelectorAll('.' + MAIN_BTN_CLASS).forEach((el) => el.remove());
        };
