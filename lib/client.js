...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      /* 1. 文件树 explorerRow 按钮（继承 _explorerRef 胶囊风格，但覆盖 margin-left 避免被推到最左） */
      .${BTN_CLASS} {
        display: inline-flex;
        align-items: center;
        gap: 4px;
      ...
      /* 6. 屏蔽官方右侧栏入口与面板（完全由 dsh-better-sidebar 接管） */
    ...
    function injectExplorerButtons() {
      ...
      for (const row of rows) {
        ...
        if (ref) {
          // 放在 ref (@文件) 后面紧随排列，ref 负责 margin-left: auto 推到右侧，idea 按钮紧挨着它
          ref.after(btn);
        } else {
          row.appendChild(btn);
        }
